	...
	Show(appointment?: Appointment, date?: number) {
		...
		if (appointment) {
			...
			if (appointment.notificationUuid) {
				...
				GetNotification(appointment.notificationUuid).then(
			...
			if (appointment.color) {
				let colorIndex = this.availableColors.findIndex(
					c => c == appointment.color
				)
				if (colorIndex !== -1) {
					...
					if (this.reminderCheckboxChecked) {
						...
						if (appointment.notificationUuid) {
							...
							if (notification != null) {
								...
								notification.Body = this.GenerateNotificationBody(
									startUnix,
									endUnix
								)
							...
							if (await SetupWebPushSubscription()) {
								let notification = new Notification({
									...
									Body: this.GenerateNotificationBody(
										startUnix,
										endUnix
									)
								})
