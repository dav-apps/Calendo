...
export class Appointment {
	...
	private async Save() {
		...
		let properties: { name: string; value: string }[] = [
			...
			{
				name: environment.appointmentStartKey,
...
export function ConvertTableObjectToAppointment(
	tableObject: TableObject
): Appointment {
	if (tableObject.TableId != environment.appointmentTableId) return null

	let name = tableObject.GetPropertyValue(
		environment.appointmentNameKey
	) as string

	let allDay: boolean = false
	let allDayString = tableObject.GetPropertyValue(
		environment.appointmentAllDayKey
	) as string
	...
	let start: number = 0
	let startString = tableObject.GetPropertyValue(
		environment.appointmentStartKey
	) as string
	if (startString != null) {
		start = +startString
	}
	...
	let end: number = 0
	let endString = tableObject.GetPropertyValue(
		environment.appointmentEndKey
	) as string
	if (endString != null) {
		end = +endString
	}

	let color = tableObject.GetPropertyValue(
		environment.appointmentColorKey
	) as string
	if (color == null) {
